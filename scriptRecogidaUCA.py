    ...
    for row in rawData:
        ...
        if place[0] == 'M':
            ...
            if place == "PTAG":
                ...
                place_num = int(place[1::])
            if not gates[place_num]:
                gates[place_num] = [[row[0], row[1], row[3]]]
            else:
                gates[place_num].append([row[0], row[1], row[3]])
...
for stand in gates:
    ...
    col += 1
    worksheet.write(row, col, f'VLC{stand_terminal}GTN{num_stand_str} 192.168.211.{num_stand + 120}', name_cells)
    if num_stand == 23:
        worksheet.write(row, col, "VLC02GTTAG 192.168.211.199", name_cells)
    ...
    col += 1
    worksheet.write(row, col, f'P{num_stand_str}', name_cells)
    num_stand += 1
...
for stand in checking:
    ...
    worksheet.write(row, col, 'Workstation ID', id_cells)
    col += 1
    worksheet.write(row, col, f'VLC{stand_terminal}CK{num_stand_str} 192.168.211.{num_stand}', name_cells)
    ...
    worksheet.write(row, col, 'ID', id_cells)
    col += 1
    worksheet.write(row, col, f'M{num_stand}', name_cells)
    num_stand += 1
